...
describe('CardItem Component', () => {
  it('should render properly and display prop data as well as gauge bar', () => {
    const wrapper = mount(CardItem, {
      props: { person: mockData, isListView: false },
      global: {
        plugins: [
          createTestingPinia({
            createSpy: vi.fn
    ...
    expect(wrapper.find('[data-testid="positive-percentage"]').text()).toEqual('38.98%')
    expect(wrapper.find('[data-testid="negative-percentage"]').text()).toEqual('61.02%')
  })

  it('should render as list when specified', () => {
    const wrapper = mount(CardItem, {
      props: { person: mockData, isListView: true },
      global: {
        plugins: [
          createTestingPinia({
            createSpy: vi.fn
  ...
  it('should let user vote once an option is selected and reflect result on gauge bar', async () => {
    ...
    expect(wrapper.find('[data-testid="positive-percentage"]').text()).toEqual('40.00%')
  })
})
